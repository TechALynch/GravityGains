  ...
  const Exercises = [
{
  ...
  name: 'Pull-Up',
  lvl: 'Beginner',
  video: 'https://www.youtube.com/watch?si=2vgszysv72W2Zgfd&v=GBqAZP6jquc&feature=youtu.be',
  techniqueSummary: 
  `
...
// // Create Exercise instances and save them to the database
// for (const ExerciseData of Exercises) {
//   const ExerciseInstance = new Exercise(ExerciseData);
...
console.log(`Workout "${pushPullWorkout.name}" created with exercises:`, exerciseIds);

// Create an array to store workout IDs
const workoutIds = [pushPullWorkout._id];

// Create the skill with the workout IDs
const pushPullSkill = await Skill.create({
  userId: User1._id, // Assuming you want to associate it with User1
  workoutID: workoutIds,
  name: 'Muscle Up',
  // You might want to set an image for the skill as well
});

console.log(`Skill "${pushPullSkill.name}" created with workouts:`, workoutIds);
